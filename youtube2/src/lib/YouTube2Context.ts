...
class YouTube2Context {
  ...
  toast(type: 'success' | 'info' | 'error' | 'warning', message: string, title = 'YouTube2') {
    this.#pluginContext.coreCommand.pushToastMessage(type, title, message);
  }

  refreshUIConfig() {
    this.#pluginContext.coreCommand.getUIConfigOnPlugin('music_service', 'youtube2', {}).then((config: any) => {
...
export default new YouTube2Context();
