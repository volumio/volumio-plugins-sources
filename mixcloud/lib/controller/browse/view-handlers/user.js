...
class UserViewHandler extends ExplodableViewHandler {
    ...
    _browseUser() {
        ...
        userModel.getUser(decodeURIComponent(view.username)).then( user => {
            ...
            return libQ.all(fetches).then( results => {
                ...
                let title = lists[0] ? (lists[0].title || '') : '';
    ...
    _getSearchParamsList(params, searchOptions) {
        ...
        if (view.inSection) {
            ...
            title = mixcloud.getI18n(UIHelper.supportsEnhancedTitles() ? 'MIXCLOUD_USERS_MATCHING' : 'MIXCLOUD_USERS_MATCHING_FULL', decodeURIComponent(view.keywords));
        }
