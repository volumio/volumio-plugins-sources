...
class BaseModel {
    ...
    _doGetItems(options, defer, currentList = [], iteration = 1) {
        ...
        fetchPromise.then( result => {
            ...
            mixcloud.getLogger().error('[mixcloud-model.base] getItems(): ' + error.message + "\n" + error.stack);
            defer.reject(error);
        });
    }
    ...
    getCacheKeyForFetch(resourceName, cacheKeyParams = {}) {
        let key = `mixcloud.model.${resourceName}`;
    ...
    cacheCloudcasts(cloudcasts) {
        cloudcasts.forEach( cloudcast => {
            mixcloud.getCache().put(this.getCacheKeyForFetch('cloudcast', { cloudcastId: cloudcast.id }), cloudcast);
        });
    }
