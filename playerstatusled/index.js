...
StatusLedController.prototype.onStart = function () {
	...
	socket.on('connect_error', (err) => {
		self.log('websocket connection error: ' + err);
		defer.reject();
	});
	
	socket.on('connect', () => {
		self.log('connected to websocket');
		socket.on('error', (err) => {self.log('websocket error: ' + err)}); // handle possible socket errors
		socket.on('pushState', self.statusChanged.bind(self)); // listen to status reports from Volumio websocket
		self.initLed();
		socket.emit('getState', ''); // make socket push status update on startup to reflect current state
		defer.resolve();
	});
