...
class SoundCloudContext {
  ...
  toast(type: 'success' | 'info' | 'error' | 'warning', message: string, title = 'SoundCloud') {
    this.#pluginContext.coreCommand.pushToastMessage(type, title, message);
  }

  refreshUIConfig() {
    this.#pluginContext.coreCommand.getUIConfigOnPlugin('music_service', 'soundcloud', {}).then((config: any) => {
  ...
  getCache() {
    if (!this.#cache) {
      this.#cache = new Cache(this.getConfigValue('cacheTTL'), this.getConfigValue('cacheMaxEntries'));
    }
    return this.#cache;
  }
...
export default new SoundCloudContext();
